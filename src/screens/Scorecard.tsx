  ...
  return (
    <Page title="АУДИТ ПРИВЫЧЕК">
        ...
        <div className="flex items-center gap-2 mt-3">
          {(['+', '=', '-'] as Verdict[]).map(v => (
            <button
              ...
              className={`btn flex-1 ${verdict === v ? (v === '+' ? 'btn-primary' : v === '-' ? 'btn-danger' : 'btn-ghost') : 'btn-ghost'} text-base`}
            >{v === '+' ? '+ ПОЛЕЗНО' : v === '-' ? '– ВРЕДНО' : '= НИКАК'}</button>
          ))}
        </div>
      ...
      <ul className="space-y-2">
        {list.map(e => (
          <li key={e.id}>
            <button onClick={() => cycle(e)} className="tile flex items-center gap-3 w-full text-left">
              <span className={`shrink-0 size-10 grid place-items-center font-display text-2xl border-2 border-ink rounded-lg ${
